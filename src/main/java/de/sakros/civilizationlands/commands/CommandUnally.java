...
public class CommandUnally implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender,Command command,String s, String[] strings) {
        if(commandSender instanceof Player){
            ...
            /*
            Utils.sendMessageFailed(player, "Dieser Befehl ist noch nicht vollständig! Aktuell untrusted er nur alle Mitglieder aus dem angegbenen Land!");
                ...
                if(landPlayer.getOwningLand() != null){
                    if(landPlayer.getOwningLand().getName().equals(playerTeam.getName())){
                        if(strings.length == 1){
                                ...
                                if(!CivilizationLands.landsAddon.getLand(strings[0]).equals(landPlayer.getOwningLand())){
                                    ...
                                    for (OfflinePlayer offlinePlayer : offlinePlayers) {
                                        if(allyLand.isTrusted(offlinePlayer.getUniqueId())){
                                            owningLand.untrustPlayer(offlinePlayer.getUniqueId());
                                            Utils.sendMessageSuccess(player, offlinePlayer.getName() + " wurde nun untrusted.");
                                        } else {
                                            Utils.sendMessageSuccess(player, offlinePlayer.getName() + " ist schon untrusted.");
                                        }
                                    }
                                    Utils.sendMessageSuccess(player, "Du hast nun alle Mitglieder vom Land " + allyLand.getName() + " untrusted.");
                                } else {
                            ...
                            Utils.sendMessageFailed(player, "Syntax Falsch! /unally <land-name>");
                        }
                    }
                } else {
